...
`AllenCahnProblem1D` - initialize the problem data structure
"""
function AllenCahnProblem1D(a, b, ϵ, nₓ, nₜ, t_max, left_bc::TLBC, right_bc::TRBC, u₀::TU) where {TLBC <: PeriodicBC, TRBC <: PeriodicBC, TU <: Function}
    @assert a < b "Cannot construct: Invalid interval!"
    @assert ϵ > 0 "Cannot construct: ϵ must be positive!"
    @assert (nₓ > 0 && nₜ > 0) "Cannot construct: nₓ and nₜ must both be positive!"
    ...
    f(x) = x - x^3
    u = u₀.(x)
    return AllenCahnProblem1D(a, b, ϵ, nₓ, Δx, x, nₜ, t_max, Δt, left_bc, right_bc, spzeros(nₓ + 1, nₓ + 1), zeros(nₓ + 1), f, u)
end